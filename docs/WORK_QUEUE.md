...
| P2 | in-progress | Add link health and catalog freshness checks | build-agent | Aug. 24 is canonical through PR #76. Branch `build/source-history-snapshot-replay-20260828` now adds a deterministic `source_check_history.py replay-snapshot` path that recomputes preserved hashes, validates source IDs/predecessors/chronology, blocks registry rewind, is idempotent, and dry-runs the actual Aug. 25 PR #62 snapshot without mutating canonical files. Require green CI and integrity review before merge; then replay Aug. 25 in a separate bounded PR, followed by Aug. 26, Aug. 27 and Aug. 28 in order. |
