...
| P2 | in-progress | Add link health and catalog freshness checks | build-agent | PR #79 merged the deterministic `source_check_history.py replay-snapshot` path. Branch `build/replay-aug25-source-health-20260828` now replays exactly four preserved Aug. 25 PR #62 observations into canonical history and advances only matching source timestamps. Require exact-head Core/source-report/maintenance/site-data verification and independent integrity review before merge. After Aug. 25 becomes canonical, process Aug. 26 PR #66 next, then Aug. 27 and Aug. 28 in order. |
