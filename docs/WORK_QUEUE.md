...
| P2 | in-progress | Add link health and catalog freshness checks | build-agent | PR #24 merged deterministic URL/source-migration diagnostics, PR #27 merged deterministic submission-phase/deadline evaluation, PR #31 merged the canonical tool-visibility regression contract, PR #34 merged evidence preservation/normalization with SHA-256 provenance, and PR #37 merged a bounded NIH source-acquisition layer. PR #50 replays the merged PR #44 raw source-health observations into canonical history/registry; after that, replay PR #47 and PR #49 snapshots in timestamp order. Broader freshness still needs additional source-specific adapters and evidence beyond simple status/deadline fields. |
| P2 | todo | Add optional news/RSS adapters with cached timestamped output | unclaimed | Keep network failures non-destructive |
| P2 | in-progress | Inventory legacy solver modules and document inputs/outputs/dependencies | build-agent | PR #39 integrated the portable 310 password-candidate solver and PR #42 integrated the portable character-region locator, both at `experimental`. PR #45 now adds a non-destructive reproduction verifier that regenerates the four alpha outputs in a temporary directory and compares them with migrated evidence in Core CI. Preserve root legacy files until reference/provenance review is complete; after independent reproduction verification, continue module-by-module. |
