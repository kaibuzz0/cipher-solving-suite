...
| P2 | in-progress | Add link health and catalog freshness checks | build-agent | PR #24 merged deterministic URL/source-migration diagnostics and PR #27 merged deterministic submission-phase/deadline evaluation. PR #31 adds reusable deterministic verification that user-visible tools flow through the canonical registry, repository browser, Pages/workspace, and Command Site snapshot; CI must pass before that verification is counted complete. A provenance-preserving acquisition/refresh policy for structured phase/deadline evidence still remains before this broader item can close. |
