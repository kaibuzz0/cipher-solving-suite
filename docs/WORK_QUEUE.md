...
| P2 | in-progress | Inventory legacy solver modules and document inputs/outputs/dependencies | build-agent | PR #39 integrated the portable 310 password-candidate solver, PR #42 integrated the portable character-region locator, and PR #45 added a non-destructive reproduction verifier that regenerates the four alpha outputs in a temporary directory and compares them with migrated evidence in Core CI. PR #48 is integrating the remaining case-local image analyzer with deterministic direct-script tests, explicit-only derived outputs, canonical tool registration, and an `experimental` claim boundary. Preserve root legacy files until reference/provenance review is complete; after independent verification, continue module-by-module. |
