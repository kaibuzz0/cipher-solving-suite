...
| P2 | in-progress | Inventory legacy solver modules and document inputs/outputs/dependencies | build-agent | `agent/310-password-candidate-solver-20260821` begins this lane by replacing the broken root 310 `brute_force.py` assumptions with a case-local deterministic candidate tool, explicit optional `pycryptodomex` dependency, direct-script tests, case linkage and canonical tool registration. Continue module-by-module; do not delete legacy code until references/provenance are checked. |
