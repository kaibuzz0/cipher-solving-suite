...
| P2 | blocked | Add link health and catalog freshness checks | build-agent | Source history is canonical through Aug. 26. PR #75 raw Aug. 27 evidence is merged on `main` but all five stored SHA-256 values fail the canonical `normalize_fingerprint(observed)` contract, so canonical replay is blocked. Preserve PR #75 unchanged, determine whether the stored hashes came from earlier observation text or the strings changed after hashing, then create an explicit provenance-preserving correction and dry-run it against Aug. 26 history. PR #82 raw Aug. 28 morning/afternoon evidence is also merged but blocked behind valid Aug. 27 replay. Open PR #85/#91/#95 remain later Aug. 29/Aug. 30 research. xTech|Search 10 remains non-actionable until its authoritative RFI/application conflict is resolved. |
